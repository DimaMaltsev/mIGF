...
public class FreeSpaceCheck_SmallGuy : Interface {
	...
	private void TriggerTypeDetectionForNeighBours(){
		...
		int b_right 	= ThereIsSomething( right 	);
		int b_left 	= ThereIsSomething( left 	);
	...
	private int ThereIsSomething( Collider2D[] others ){ // 0 - static block, 1 - moving block, -1 - nothing
		if( others != null ){
			for( int i = 0 ; i < others.Length; i++ ){
				...
				if( block.GetComponent<Box_Moves>() != null && block.GetComponent<ObjectController>().propertyFacade.GetPropertyNumber("sy")!=0) return 1;
				if( block.GetComponent<Box_Moves>() != null && block.GetComponent<ObjectController>().propertyFacade.GetPropertyNumber("sy")==0) return 0;
